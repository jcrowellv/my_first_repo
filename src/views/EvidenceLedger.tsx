...
export function EvidenceLedger() {
  ...
  const evidence = useMemo(() => [...canonical.evidence]
    ...
    .sort((a, b) => diagnosticityOrder[b.diagnosticity] - diagnosticityOrder[a.diagnosticity] || b.date.localeCompare(a.date)), [milestone, diagnosticity, sourceType]);
        ...
        {evidence.map((item) => (
          <details key={item.id} className={`group rounded-2xl border border-line bg-panel shadow-instrument ${item.diagnosticity === "low" ? "opacity-80" : ""}`}>
            <summary className="cursor-pointer list-none p-5 md:p-6">
              ...
              <div className="mt-4 flex flex-wrap gap-1.5">{item.milestone_tags.map((tag) => <span key={tag} className="rounded-full bg-raised px-2.5 py-1 font-mono text-[9px] text-muted">{milestonesById.get(tag)?.code}</span>)}</div>
            </summary>
